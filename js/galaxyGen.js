                                            // GALAXY GEN BY CHRIS GODBER ^_^ 
                                            // A-FRAME COMPONENT LIBRARY FOR GENEREATING GALAXY SCENES 0_0
                                            
// script for generating buildings / game logic etc
// componenet library for galaxygen 
// global game vars
...
function beginGame() {
    ...
    let starFieldContainer = document.createElement('a-entity');
    starFieldContainer.setAttribute('id', 'starField');
    spaceScene.appendChild(starFieldContainer);

    createRandomDebris(2);
    createStarFields(starsNum);
    createNebulae(2);
...
function newScene() {
    ...
    let starFieldContainer = document.createElement('a-entity');
    starFieldContainer.setAttribute('starField');
    starFieldContainer.setAttribute('id', 'starField');
    spaceScene.appendChild(starFieldContainer);

    // generate star system bg elements
    createStarFields(starsNum);
...
function createRandomDebris(fieldsNum) {
    ...
    for (i = 0; i < fieldsNum; i++) {
        ...
        for (i = 0; i < randomDebrisNum; i++) {
            ...
            let asteroidSystems = document.getElementById("asteroidSystems")
            asteroidSystems.appendChild(asteroidContainer);
        }
    }
}

// create random star fields
function createStarFields(starsNum) {
    console.log('stars starfield calc'+starsNum);
    let i;
    initStarfield = true;
    // for setting x pos after first star field gen
    let initPos = 0;

    for (i = 0; i < starsNum*2; i++) {
        let starField = document.createElement('a-entity');

            let fieldPosxP = initPos+getRandomInt(-10000, 50000);
            let fieldPoszP = getRandomInt(-10000, 100000);
            let fieldPosyP =getRandomInt(-10000, 50000);

        // starField.setAttribute('star-system', 'radius:'+ getRandomInt(1000, 12000),'depth'+getRandomInt(1000, 12000),'number',getRandomInt(3000, 10000));
        starField.setAttribute('star-system',' count:2000; radius: 20000; depth: 1200');
        starField.setAttribute('id','Newstarfield'+i);
        // set starfield positioning
        starField.setAttribute('position', {x: fieldPosxP, y: fieldPosyP, z: fieldPoszP});
        let starFieldContainer = document.getElementById("starField");
        starFieldContainer.appendChild(starField);
        }
}

// create random Nebuale
function createNebulae(amountNebulae) {
    console.log('stars rand nebulae calc'+amountNebulae);
    let i;
    // for setting x pos after first star field gen
    let initPos = 0;
    for (i = 0; i < amountNebulae; i++) {
        let nebPlane = document.createElement('a-box');
        let fieldPosxP = initPos+getRandomInt(-15000, 15000);
        let fieldPoszP = getRandomInt(-15000, 15000);
        let fieldPosyP =getRandomInt(-15000, 15000);
        let nebulaeFX = document.createElement('a-image');
        nebulaeFX.setAttribute('width', getRandomInt(500, 8000));
        nebulaeFX.setAttribute('height', getRandomInt(500, 15000));
        nebulaeFX.setAttribute('look-at', '#rig');
        nebulaeFX.setAttribute('src', '#neb1');
        nebulaeFX.setAttribute('neb','');
        nebulaeFX.setAttribute('position', {x: fieldPosxP, y: fieldPosyP, z: fieldPoszP});
        // nebulaeFX.setAttribute('material', 'transparent: true; opacity: 1.0; alphaTest: 0.01;');
        // nebPlane.setAttribute('height', 5);
        // nebPlane.setAttribute('width', 10);
        // nebPlane.setAttribute('position', {x: fieldPosxP, y: fieldPosyP, z: fieldPoszP});
        const spaceScene = document.getElementById("spaceScene");
        // nebPlane.appendChild(nebulaeFX);
        spaceScene.appendChild(nebulaeFX);
    }
}
